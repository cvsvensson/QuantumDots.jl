...
tuplify(μ::Number,N) = fill(μ,N)
tuplify(μ::NTuple{M},N) where M = (@assert M==N; μ)
function BD1_hamiltonian(c::FermionBasis{M}; μ, h, t, Δ,Δ1, U, V, dθ, dϕ) where M
    ...
    θ = fill(dθ,N) .* (1:N .- 1)
    ϕ = fill(dϕ,N) .* (1:N .- 1)
    _BD1_hamiltonian(c::FermionBasis{M}; μ = tuplify(μ,N), h = tuplify(h,N), t = tuplify(t,N), Δ = tuplify(Δ,N),Δ1 = tuplify(Δ1,N), U = tuplify(U,N), V = tuplify(V,N), θ, ϕ)
end
