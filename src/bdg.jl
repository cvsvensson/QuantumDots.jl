...
#     U*bdgham*U'
# end
function majoranas(quasiparticle::QuasiParticle)
    N = nbr_of_fermions(quasiparticle.basis)
    i = Matrix(I/sqrt(2),N,N)

    maj1 = quasiparticle.weights.values + quasiparticle_adjoint(quasiparticle.weights.values, N)
    maj2 = quasiparticle.weights.values - quasiparticle_adjoint(quasiparticle.weights.values, N)
    # U = [i i; i -i]
    return MajoranaQuasiParticle(maj1,quasiparticle.basis), MajoranaQuasiParticle(maj2,quasiparticle.basis)
